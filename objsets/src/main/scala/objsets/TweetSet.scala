...
class Empty extends TweetSet {
  ...
  override def union(acc: TweetSet): TweetSet = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweetAcc(acc: TweetList): TweetList =
    new Cons(mostRetweeted, this.remove(mostRetweeted).descendingByRetweetAcc(acc))
  ...
  override def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      if (right.mostRetweeted.retweets < elem.retweets) elem else right.mostRetweeted
    } else {
      if (left.mostRetweeted.retweets < elem.retweets) elem else left.mostRetweeted
    }
  }
  ...
  override def union(acc: TweetSet): TweetSet =
    acc.union(left).union(right).incl(elem)
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(t => google
      .exists(keyword => t.text.contains(keyword)))
...
//  val set1 = new Empty().incl(new Tweet("a", "a body", 20))
//  val set2 = new Empty().incl(new Tweet("b", "b body", 30))
